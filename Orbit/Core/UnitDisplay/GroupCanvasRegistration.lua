	...
	return function(comp, anchorX, anchorY, offsetX, offsetY, justifyH)
		...
		if Txn and Txn:IsActive() and Txn:GetPlugin() == plugin then
			...
			for _, subKey in ipairs(STATUS_GROUP_KEYS) do
				Txn:SetPosition(subKey, posData)
			end
		else
			local positions = plugin:GetComponentPositions(1)
...
do
	local HR = Orbit.HealerAuraRegistry
	if HR then
		for _, k in ipairs(HR:AllSlotKeys()) do
			AURA_ICON_KEYS[k] = true
		end
	end
end
function Reg:RegisterComponents(plugin, container, firstFrame, textKeys, iconKeys, auraBaseIconSize)
	if not OrbitEngine.ComponentDrag or not firstFrame then
		return
	end
	...
	for _, key in ipairs(iconKeys) do
		...
		if element then
			...
			OrbitEngine.ComponentDrag:Attach(element, container, {
				key = key,
				isAuraContainer = isAura or false,
				onPositionChange = isAura and MakeTierAuraPositionCallback(plugin, key)
					or MakeTierPositionCallback(plugin, key),
			})
		end
	end
	...
	for _, frame in ipairs(frames) do
		...
		for _, iconKey in ipairs(iconKeys) do
			if frame[iconKey] and savedPositions[iconKey] then
				...
				local selfAnchor = isAura
						and BuildSelfAnchor(false, true, pos.selfAnchorY or anchorY, pos.justifyH or "CENTER")
					or "CENTER"
...
function Reg:PrepareIcons(plugin, frame, cfg, healerSlots, raidBuffs)
	...
	for _, slot in ipairs(healerSlots) do
		...
		if OrbitEngine.ComponentDrag and not icon._canvasAttached then
			...
			OrbitEngine.ComponentDrag:Attach(icon, container, {
				key = slot.key,
				isAuraContainer = true,
				onPositionChange = MakeTierAuraPositionCallback(plugin, slot.key),
			})
		end
	end
	if raidBuffs and #raidBuffs > 0 then
		...
		if OrbitEngine.ComponentDrag and not rb._canvasAttached then
			...
			OrbitEngine.ComponentDrag:Attach(rb, container, {
				key = "RaidBuff",
				isAuraContainer = true,
				onPositionChange = MakeTierAuraPositionCallback(plugin, "RaidBuff"),
			})
		end
	end
end
	...
	if isCanvasMode then
		...
		for _, entry in ipairs(auraIconEntries) do
			...
			if btn and not isDisabled(entry.key) then
				...
				if Orbit.Skin and Orbit.Skin.Icons then
					Orbit.Skin.Icons:ApplyCustom(btn, Orbit.Constants.Aura.SkinNoTimer)
				end
				btn:Show()
			elseif btn then
				btn:Hide()
		...
		for _, slot in ipairs(healerSlots) do
			if not isDisabled(slot.key) then
				...
				if Orbit.Skin and Orbit.Skin.Icons then
					Orbit.Skin.Icons:ApplyCustom(hIcon, Orbit.Constants.Aura.SkinNoTimer)
				end
				hIcon:Show()
			elseif frame[slot.key] then
				frame[slot.key]:Hide()
			end
		end
		if raidBuffs and #raidBuffs > 0 and not isDisabled("RaidBuff") then
			...
			local rb = plugin:EnsureRaidBuffContainer(frame, "RaidBuff", raidBuffs, rbSize)
			if rbPos then
				ApplyTextPosition(rb, frame, rbPos, nil, nil, nil, true)
			end
			rb:Show()
		elseif frame.RaidBuff then
			frame.RaidBuff:Hide()
		end
	else
		local hideKeys = cfg.hideKeys
			or {
				"PhaseIcon",
				"ReadyCheckIcon",
				"ResIcon",
		...
		for _, key in ipairs(healerKeys) do
			if frame[key] then
				frame[key]:Hide()
			end
		end
