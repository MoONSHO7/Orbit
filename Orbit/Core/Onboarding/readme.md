...
- `TourPlugin.lua` registers `"Orbit_Tour"` via `Orbit:RegisterPlugin`, creates frames with `FrameFactory:Create`, defines `AddSettings` with `SchemaBuilder` (width/height/opacity)
- `EditModeTour.lua` drives the tour flow: dark overlay, 6 sequential stops, task-gated Next buttons, snap isolation, drag/anchor/nudge tracking
