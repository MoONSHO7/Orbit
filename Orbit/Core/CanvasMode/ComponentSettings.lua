...
local COMPONENT_TITLES = {
    ...
    BuffBarName = "Buff Bar Name",
    BuffBarTimer = "Buff Bar Timer",
}
...
function Settings:Open(componentKey, container, plugin, systemIndex)
    ...
    local function GetValueFromVisual(cont, key)
        ...
        elseif key == "IconSize" then
            return cont:GetWidth() and math.floor(cont:GetWidth() + 0.5)
        end
        return nil
    end
...
function Settings:ApplyStyle(container, key, value)
    ...
    if key == "IconSize" then
        ...
        elseif self.container then
            ...
            if self.container.visual and self.container.visual.SetSize and not self.container._cyclingTicker then self
                    .container.visual:SetSize(value, value) end
