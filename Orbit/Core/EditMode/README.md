...
Because the preview lines signal where the frame WILL snap on drop, `Frame/Snap.lua:DetectSnap` also appends UIParent-edge and grid-line offsets as synthetic alignment candidates (read from `EditModeMagnetismManager.magneticGridLines`, which the Blizzard grid populates in `EditModeGridLineMixin:SetupLine`). These use Blizzard's own `magnetismRange` (8 screen pixels, matches the red-line tolerance). When a UIParent/grid candidate is within `magnetismRange` on an axis, it **overrides** any frame-to-frame snap on that axis — the red line is a visual contract, so honoring it takes priority over Orbit's tighter 5px frame-to-frame match. Anchor candidates on the overridden axis are cleared at the same time so `Drag.lua`'s drop path doesn't pull the frame back to an edge target it's no longer visually pointing at. On drop, the final L/B is pushed through `Pixel:Snap` so the frame edge lands exactly on the guideline's pixel. This keeps the visual promise (red line) and the drop behavior (actual snap) in lockstep.
