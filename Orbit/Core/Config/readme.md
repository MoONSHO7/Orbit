...
| Panels/Tabs/GlobalTab.lua | Global tab schema: font, border style, `IconBorderStyle`. The built-in "Orbit" style (`value="orbit"`) shows Border Thickness + Corner Roundness sliders; LibSharedMedia styles show Border Edge Size + Border Offset instead. The Border Style / Icon Border Style dropdowns place the "Orbit" entry above a divider, LibSharedMedia borders below it. The Font / Border Style / Icon Border Style rows carry a value-column color swatch (`valueColor`) for `FontColorCurve` / `BorderColor` / `IconBorderColor`; the border swatches hide for LibSharedMedia styles. |
