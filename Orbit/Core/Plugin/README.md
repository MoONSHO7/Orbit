...
| ProfileManager.lua | profile crud (create, copy, delete, switch). fires `ORBIT_PROFILE_CHANGED`. owns the per-profile **Visibility** blob (`profile.Visibility = { frames, fade }`) via `GetActiveVisibility` (lazy-seeded, resolved each access) and the one-time `MigrateVisibilityToProfiles` (relocates the old account-wide `VisibilityEngine`/`FadeProfiles` roots into each profile + strips legacy per-plugin visibility keys). |
