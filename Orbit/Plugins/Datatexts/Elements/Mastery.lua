...
local W = DT.BaseDatatext:New("Mastery", L.PLU_DT_MASTERY_NAME)
...
function W:Update()
    ...
    if self.showPercentage then
        if pct then self:SetText(L.PLU_DT_STAT_MASTERY_LABEL .. string.format("|cffffffff%.2f%%|r", pct))
        else self:SetText(L.PLU_DT_STAT_MASTERY_LABEL .. "|cffffffff" .. L.CMN_HIDDEN_VALUE .. "|r") end
    else
        if rating then self:SetText(L.PLU_DT_STAT_MASTERY_LABEL .. string.format("|cffffffff%d|r", rating))
        else self:SetText(L.PLU_DT_STAT_MASTERY_LABEL .. "|cffffffff" .. L.CMN_HIDDEN_VALUE .. "|r") end
    end
end
