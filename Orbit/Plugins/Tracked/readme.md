...
| TrackedIconItem.lua | factory for individual icon-item buttons (icon + cooldown swipe + charge text + drop highlight). pure factory, stateless, container owns the ticker. owns `ApplyCanvasComponents` for per-icon `ChargeText` positioning + `_chargeTextDisabled` caching |
| TrackedContainer.lua | icons-mode container frame. sparse 2D grid, neighbor-expansion drop zones, drop receive pipeline, per-container cursor watcher and ticker, shift-right-click empty container → delete. installs `frame:CreateCanvasPreview` (via shared `IconCanvasPreview`) for the canvas dialog |
| TrackedBar.lua | single-payload bar frame. accepts spells OR items, renders in one of three modes (charges / active+cd / cd-only) determined at drop time. two-step shift-right-click ladder (clear payload → delete bar), drop hint texture states. owns `ApplyCanvasComponents` for `NameText`/`CountText` and installs `frame:CreateCanvasPreview` sized to the inner StatusBar width |
