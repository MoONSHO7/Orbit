...
| TrackedIconItem.lua | factory for individual icon-item buttons (icon + cooldown swipe + reverse active swipe + charge text + drop highlight). stateless factory, container owns the ticker. full cooldown display: phase-aware desaturation/alpha curves via `BuildPhaseCurve`, `ActiveCooldown` reverse swipe during the active phase (cast-driven — `_activeDuration` learned from the spell's live SELF aura on first cast via `CooldownData:ResolveActiveDuration` + `CooldownLearn`, `_activeGlowExpiry` set on cast), charge caching with `issecretvalue` guard + `TrackChargeCompletion`, `useSpellId` fallback for items, non-usable item display. per-icon proc/active glows + sounds via `Orbit.SpellGlows` (shift-right-click menu). `Update` returns the resolved state ("ready"/"cooldown"/"active") stored on `icon._lastState` so the container's visual poll can skip ready icons. owns `ApplyCanvasComponents` for per-icon `ChargeText` positioning + `_chargeTextDisabled` caching. NOTE: an active phase for TARGET debuffs (e.g. Haunt) is **not** rendered here — see "Why target-debuff active phases aren't shown" below. |
| TrackedContainer.lua | icons-mode container frame. sparse 2D grid, neighbor-expansion drop zones, drop receive pipeline, per-container cursor watcher and event-driven update (SPELL_UPDATE_COOLDOWN, BAG_UPDATE_COOLDOWN, BAG_UPDATE, SPELLS_CHANGED; + 0.3s visual poll, gated per-icon on `_lastState ~= "ready"` so idle icons don't allocate from C_Spell calls). spell cast watcher (UNIT_SPELLCAST_SUCCEEDED → `_activeGlowExpiry` for the cast-driven active phase + active-condition callout via `SpellGlows:FireAlert` + `OnChargeCast` charge bookkeeping). equipment change handler (PLAYER_EQUIPMENT_CHANGED → item slot refresh). talent reparse (TRAIT_CONFIG_UPDATED → `ReparseActiveDurations` → rebuild phase curves). per-icon shift-right-click → `SpellGlows:OpenMenu` (proc/active glow + sound); shift-right-click empty container → delete. installs `frame:CreateCanvasPreview` (via shared `IconCanvasPreview`) for the canvas dialog |
| TrackedBar.lua | single-payload bar frame. accepts spells OR items, renders in one of three modes (charges / active+cd / cd-only) determined at drop time. event-driven 0.1s update ticker (started by SPELL_UPDATE_COOLDOWN / SPELL_UPDATE_CHARGES / BAG_UPDATE_COOLDOWN / UNIT_SPELLCAST_SUCCEEDED, self-suspends when `RefreshSpellState` returns "ready"). spell name + icon cached per-payload in `CacheImmutableState` (called from `Apply`) so the ticker only allocates per-tick C_Spell calls while a cooldown is actively running. two-step shift-right-click ladder (clear payload → delete bar), drop hint texture states. owns `ApplyCanvasComponents` for `NameText`/`CountText` and installs `frame:CreateCanvasPreview` sized to the inner StatusBar width |
...
| TrackedTour.lua | single-shot teach tooltip pinned alongside the Tracked Icons / Tracked Bars side tabs the first time the user opens `CooldownViewerSettings`. anchors `TOPLEFT → TOPRIGHT` of the icons tab via `CVE:GetTab("Orbit_Tracked.Icons")` so the tooltip naturally sits adjacent to both stacked tabs. wired through `EventRegistry:RegisterCallback("CooldownViewerSettings.OnShow"/"OnHide", ...)`; account-wide flag `Orbit.db.AccountSettings.TrackedTabsHintComplete` flips on first hide. strings live in `Localization/Domains/Tours.lua` under the `TOUR_CDM_TRACKED_TABS_*` keys. |
| Tracked.xml | load order: TrackedPlugin → TrackedIconItem → TrackedContainer → TrackedBar → TrackedSettings → TrackedTour |
...
both modes set `anchorOptions.mergeBorders = true`. when two tracked frames (or a tracked frame and any other orbit frame with the same flag) are anchored at zero padding, `Orbit.Skin:UpdateGroupBorder` walks the chain and draws a single wrapper border around the merged group instead of one border per frame. icon containers also set `frame._isIconContainer = true` so the wrapper picks the **Global Icon Border Style** (matching the cooldown manager's essential/utility group). bars do not flag themselves as icon containers, so a chain of bars uses the regular **Global Border Style** — same behavior as the cooldown manager's BuffBar group.
...
## docked bar width / height

bars set both `frame.orbitWidthSync = true` and `frame.orbitHeightSync = true`, so the cross-axis dimension is propagated from the parent through `Anchor:SyncChild`:

- TOP/BOTTOM dock (vertical chain) → parent's width syncs to child's width.
- LEFT/RIGHT dock (horizontal chain) → parent's height syncs to child's height.

`Bar:Apply` would normally call `frame:SetSize(savedWidth, savedHeight)` and clobber the synced dimension — leaving dividers and recharge geometry stuck at the saved size while the bar visually grows. so Apply branches on the anchor edge:

- horizontal dock (`anchor.edge == LEFT|RIGHT`) → `SetWidth(savedWidth)` only, read `frame:GetHeight()` for downstream layout.
- vertical dock (`anchor.edge == TOP|BOTTOM`) → `SetHeight(savedHeight)` only, read `frame:GetWidth()` for downstream layout.
- undocked → `SetSize(savedWidth, savedHeight)`.

this matches the buff-bar pattern in `CooldownLayout` ("when docked, anchor cross-axis is authoritative"). the Width / Height sliders in the settings panel still apply when the bar is undocked; once it joins a chain, the chain's cross-axis dim wins.
...
vertical bars set `StatusBar:SetOrientation("VERTICAL")` plus the same on `RechargePositioner` and `RechargeSegment`. they also set `SetRotatesTexture(true)` on the visible bars (`StatusBar`, `RechargeSegment`) so a horizontally-authored statusbar texture is rotated 90° instead of stretched down the long axis; horizontal bars set it back to `false`. WoW's vertical fill is `BOTTOM→TOP`, which naturally gives the requested behavior in both continuous modes:
...
`orbitResizeBounds` flips its min/max per orientation (horizontal: 80-400 W × 12-40 H; vertical: 12-40 W × 80-400 H). Since `Width`/`Height` are literal screen dimensions, the resize handle writes directly to them — no `widthKey`/`heightKey` remap needed. `anchorOptions` is `{ horizontal = true, vertical = true }` so bars can be chained on either axis: TOP/BOTTOM stacking shares width via `orbitWidthSync`, LEFT/RIGHT side-by-side shares height via `orbitHeightSync`. natural chains pair with the bar's orientation — horizontal bars stack TOP/BOTTOM (long axis aligned), vertical bars sit LEFT/RIGHT (long axis aligned) — but mixed chains still render correctly; the cross-axis sync just propagates the wrong dim if you mix orientations in a chain.
...
`Bar:SetBarColorState(frame, dim)` is the single switch — it caches `_barColorIsDim` and only re-vertex-colors the StatusBar texture when the state actually flips, so the 0.1s ticker is a no-op when the bar isn't crossing a phase boundary. the dim multiplier is the same `RECHARGE_DIM = 0.35` used by the `RechargeSegment` in charges mode, so "filling toward ready" reads with the same darker tint regardless of which render mode is on screen:
...
## why target-debuff active phases aren't shown (and the reparent strategy we rejected)

A Tracked icon's active phase is **cast-driven** and works only for SELF auras (player buffs/self-debuffs): on cast we set `_activeGlowExpiry = now + _activeDuration` (the latter learned from the live self aura) and run the reverse swipe for that window. We deliberately do **not** render an active phase for a **target debuff** (e.g. Affliction Haunt on the boss), and it cannot be done from this addon-drawn surface:

- In combat, a target debuff's `aura.expirationTime` / `duration` / `spellId` are **secret values**. Any Lua arithmetic/comparison on them (`expirationTime - GetTime()`, matching `aura.spellId` to identify which instance is ours) **throws** from tainted addon code. The secret-safe sink `Cooldown:SetCooldownFromDurationObject(C_UnitAuras.GetAuraDuration(unit, auraInstanceID))` still needs the `auraInstanceID`, which you can only get by a secret `spellId` comparison first. There is no addon-reachable path.
- Blizzard's CooldownViewer **can** render it, because it runs in a secure/untainted context where the secret values flow without tripping the barrier. So the live target-debuff timer is available **only** on the native CooldownViewer bars (Essential / BuffIcon / BuffBar), which Orbit's `CooldownManager` skins in place — that is where users should keep such spells.

**The reparent strategy (investigated, not adopted):** to host that live native rendering inside a Tracked cell, you would reparent the native `CooldownViewer` item frame (a plain insecure `Frame`, `SetParent`-safe in/out of combat — no taint) into the cell and skin it. The blocker is **not** taint — it is that the native viewer still **owns and pools** the frame: `RefreshLayout`/`RefreshData` reclaim and re-anchor it every cooldown/aura change, so pulling one frame out **desyncs the pool, the viewer layout, and the Cooldown Settings panel** (icons land in the wrong slots; the settings UI our Tracked tabs hang off of gets "mixed up"). Coolinator avoids this only by **hiding the native viewers entirely and owning the whole layout** — i.e. *disable Blizzard's viewers and reparent every frame into our own frames*. For Orbit that means rebuilding `CooldownManager` to **replace** the native viewers instead of skinning them in place — at which point **we lose control of / supplant the Cooldown Settings panel** (it is Blizzard's EditMode surface). That is the only way to get combat-correct target-debuff active phases in a Tracked-style grid, and it is a major `CooldownManager` rewrite deferred until there's appetite for it. Confirming notes: native item frames are insecure (`CooldownViewer.xml` BaseItemTemplate is a bare `<Frame>`); a custom/standalone viewer does NOT help (its item mixin does the same secret Lua math and throws when addon-driven); "Not Displayed" (HiddenSpell/HiddenAura category) cleanly removes a spell from the CDM but leaves **no** frame to host.
