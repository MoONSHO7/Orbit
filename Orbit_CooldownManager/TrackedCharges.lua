...
local function IsChargeSpell(spellId)
    if not spellId or not C_Spell.GetSpellCharges then return false, nil end
...
local function GetBarColor(plugin, sysIndex)
    ...
    local c = curveData and OrbitEngine.WidgetLogic and OrbitEngine.WidgetLogic:GetFirstColorFromCurve(curveData)
...
local function UpdateChargeBarLabel(frame)
    ...
    if selection.Label then selection.Label:SetText(label) end
    if selection.system then selection.system.GetSystemName = function() return label end end
end
...
function Plugin:CreateChargeBarFrame(name, systemIndex, label)
    ...
    frame:HookScript("OnSizeChanged", function()
        ...
        if frame.chargeSpellId then plugin:LayoutChargeBar(frame) end
    end)
    ...
    frame:SetScript("OnReceiveDrag", function() plugin:OnChargeFrameDrop(frame) end)
...
function Plugin:CreateChargeControlButtons(anchor)
    ...
    plusBtn:SetScript("OnClick", function() if not InCombatLockdown() then plugin:SpawnChargeChild() end end)
    ...
    minusBtn:SetScript("OnClick", function()
        ...
        if anchor.isChildChargeBar then plugin:DespawnChargeChild(anchor) end
    end)
...
function Plugin:RefreshAllChargeControlVisibility()
    ...
    for _, childData in pairs(self.activeChargeChildren) do
        if childData.frame then self:UpdateChargeControlVisibility(childData.frame) end
    end
end

function Plugin:UpdateAllChargeControlColors()
    local count = 0
    for _ in pairs(self.activeChargeChildren) do count = count + 1 end
...
function Plugin:SpawnChargeChild()
    local count = 0
    for _ in pairs(self.activeChargeChildren) do count = count + 1 end
    if count >= MAX_CHARGE_CHILDREN then return nil end

    local slot = nil
    for s = 1, MAX_CHARGE_CHILDREN do
        if not self.activeChargeChildren["charge:" .. s] then slot = s; break end
    end
    ...
    if viewerMap then viewerMap[systemIndex] = { anchor = frame, isChargeBar = true } end
...
function Plugin:DespawnChargeChild(frame)
    if not frame or not frame.isChildChargeBar then return end
    ...
    if self.viewerMap then self.viewerMap[systemIndex] = nil end
...
function Plugin:BuildChargeButtons(frame, maxCharges)
    ...
    for i = maxCharges + 1, #frame.buttons do frame.buttons[i]:Hide() end
end
...
function Plugin:LayoutChargeBar(frame)
    ...
    if frame.chargeSpellId then
        ...
        local barColor = GetBarColor(self, sysIndex)
        local bgColor = GetBgColor()

        self:SkinChargeButtons(frame, maxCharges, width, height, borderSize, spacing, texture, barColor, bgColor, scale)
...
function Plugin:LayoutChargeBars()
    ...
    for _, childData in pairs(self.activeChargeChildren) do
        if childData.frame then self:LayoutChargeBar(childData.frame) end
    end
end

function Plugin:SkinChargeButtons(frame, maxCharges, totalWidth, height, borderSize, spacing, texture, barColor, bgColor, scale)
    ...
    for i = 1, maxCharges do
        ...
        btn.bg:SetColorTexture(bgColor.r, bgColor.g, bgColor.b, bgColor.a or 0.5)
        ...
        if not btn.orbitBackdrop then
            ...
            if btn.orbitBackdrop.SetBackdrop then btn.orbitBackdrop:SetBackdrop(nil) end
        end
    ...
    if overrides.Font and LSM then fontPath = LSM:Fetch("font", overrides.Font) or fontPath end
    ...
    if overrides.ShowShadow then
        frame.CountText:SetShadowOffset(1, -1)
        frame.CountText:SetShadowColor(0, 0, 0, 1)
    else
        frame.CountText:SetShadowOffset(0, 0)
    end
    if ApplyTextPosition then ApplyTextPosition(frame.CountText, frame, pos) end
end
...
function Plugin:SetupChargeBarCanvasPreview(frame, sysIndex)
    ...
    frame.CreateCanvasPreview = function(self, options)
        ...
        local maxCharges = 3
        local segmentWidth = width / maxCharges
        for i = 1, maxCharges do
            local seg = CreateFrame("Frame", nil, preview, "BackdropTemplate")
            seg:SetSize(segmentWidth, height)
            seg:SetPoint("LEFT", preview, "LEFT", (i - 1) * segmentWidth, 0)

            local bg = seg:CreateTexture(nil, "BACKGROUND")
            bg:SetAllPoints()
            bg:SetColorTexture(bgColor.r, bgColor.g, bgColor.b, bgColor.a or 0.5)

            local fill = seg:CreateTexture(nil, "ARTWORK")
            fill:SetAllPoints()
            local fillAlpha = (i <= 2) and 1 or 0.3
            fill:SetColorTexture(barColor.r, barColor.g, barColor.b, fillAlpha)
            ...
            seg:SetBackdrop(backdrop)
            seg:SetBackdropColor(0, 0, 0, 0)
            if borderSize > 0 then seg:SetBackdropBorderColor(0, 0, 0, 1) end
        end
        ...
        if CreateDraggableComponent then
            ...
            if comp then comp:SetFrameLevel(preview:GetFrameLevel() + 10); preview.components["ChargeCount"] = comp; fs:Hide() end
        else
            fs:ClearAllPoints()
...
function Plugin:OnChargeFrameDrop(frame)
    ...
    if not actualId then return end
...
function Plugin:UpdateChargeFrame(frame)
    if not frame or not frame.chargeSpellId then return end
    local chargeInfo = C_Spell.GetSpellCharges(frame.chargeSpellId)
    if not chargeInfo then return end

    for _, btn in ipairs(frame.buttons) do btn.Bar:SetValue(chargeInfo.currentCharges) end
...
function Plugin:StartChargeUpdateTicker()
    ...
    self.chargeUpdateTicker = C_Timer.NewTicker(UPDATE_INTERVAL, function()
        local anchor = self.chargeBarAnchor
        if anchor and anchor:IsShown() and anchor.chargeSpellId then self:UpdateChargeFrame(anchor) end
        for _, childData in pairs(self.activeChargeChildren) do
            local f = childData.frame
            if f and f:IsShown() and f.chargeSpellId then self:UpdateChargeFrame(f) end
        end
    end)
end
...
function Plugin:UpdateSeedVisibility(frame)
    if not frame or not frame.SeedButton then return end

    local hasSpell = frame.chargeSpellId ~= nil

    local isDraggingCharge = false
    local spellId = ResolveSpellFromCursor()
    if spellId then isDraggingCharge = IsChargeSpell(spellId) end

    local isEditMode = EditModeManagerFrame and EditModeManagerFrame:IsShown()

    if frame.DropHighlight then frame.DropHighlight:SetShown(hasSpell and isDraggingCharge) end

    if hasSpell then frame.SeedButton:Hide(); return end

    if isEditMode or isDraggingCharge then
        local w, h = frame:GetWidth(), frame:GetHeight()
        local plusSize = math.min(w, h) * SEED_PLUS_RATIO
        if OrbitEngine.Pixel then plusSize = OrbitEngine.Pixel:Snap(plusSize) end
        frame.SeedButton.Plus:SetSize(plusSize, plusSize)
        frame.SeedButton:Show()
        if isDraggingCharge then frame.SeedButton.PulseAnim:Play() else frame.SeedButton.PulseAnim:Stop(); frame.SeedButton.Glow:SetAlpha(0.6) end
    else
...
function Plugin:UpdateAllSeedVisibility()
    ...
    for _, childData in pairs(self.activeChargeChildren) do
        if childData.frame then self:UpdateSeedVisibility(childData.frame) end
    end
end
...
function Plugin:ReloadChargeBarsForSpec()
    ...
    if anchor then
        ...
        for _, btn in ipairs(anchor.buttons) do btn:Hide() end
    end
    ...
    for key, childData in pairs(self.activeChargeChildren) do
        if childData.frame then
            ...
            for _, btn in ipairs(childData.frame.buttons) do btn:Hide() end
...
function Plugin:RestoreChargeSpell(frame, sysIndex)
    ...
    if isCharge then data.maxCharges = ci.maxCharges end
...
function Plugin:RestoreChargeBars()
    ...
    local viewerMap = self.viewerMap
    if viewerMap then viewerMap[CHARGE_BAR_INDEX] = { anchor = anchor, isChargeBar = true } end
...
function Plugin:RegisterChargeRechargeWatcher()
    if self._chargeRechargeWatcherSetup then return end
    ...
    frame:SetScript("OnEvent", function(_, _, unit, _, spellId)
        ...
        local function HandleCast(chargeFrame)
            if not chargeFrame or chargeFrame.chargeSpellId ~= spellId then return end
            if not chargeFrame._trackedCharges or chargeFrame._trackedCharges <= 0 then return end
